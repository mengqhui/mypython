# -*- coding: utf-8 -*-
# 涵盖各种分类，包括商品名、卖家id、地区、价格等信息，json格式。
import itertools
import json
import re
import sys
    ...
    while True:
        try:
            for key, value in db_cate.RangeIter():
                if value != b'OK':
                    print('CateThread: put {} into queue'.format(
                        key.decode('utf-8')))
...
if __name__ == '__main__':
    ...
    cate_th.start()
    item_th = [Thread(target=item_thread, args=(
        cate_queue, db_cate, db_item)) for _ in range(5)]
